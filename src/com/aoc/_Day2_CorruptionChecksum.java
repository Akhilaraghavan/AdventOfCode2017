...
public class _Day2_CorruptionChecksum {

	public static final String input = "104	240	147	246	123	175	372	71	116	230	260	118	202	270	277	292\r\n" + 
	...
	public static void main(String args[]) {
		...
		int checksum = Stream.of(rows).mapToInt(row -> {
		...
		int divisibleChecksum =  Stream.of(rows)
			.mapToInt(row -> {
				String[] columns = row.split("\\s");
				int[] values = new int[columns.length];
				int i = 0;
				for (i = 0; i<columns.length; i++) {
					int current = values[i] = (values[i] == 0) ? Integer.valueOf(columns[i]) : values[i]; 
					for (int j = i+1; j<columns.length; j++) {
						int next = values[j] = (values[j] == 0) ? Integer.valueOf(columns[j]) : values[j]; 
		...
		System.out.println(divisibleChecksum);
	}
}
